...
export default function Education() {
  ...
  return (
    <section id="education" ref={sectionRef} className="section relative z-10 py-24">
      <div className="max-w-5xl mx-auto w-full px-6 md:px-0">
        ...
        <div className="relative pl-8 md:pl-0">
          ...
          {/* Vertical Line */}
          <div className="absolute left-8 md:left-[2.5rem] top-4 bottom-4 w-px bg-gradient-to-b from-white/20 via-white/10 to-transparent" />
          ...
          <div className="flex flex-col gap-12">
            {EDUCATION_DATA.map((item, index) => (
              ...
              >
                ...
                {/* Timeline Node */}
                <div 
                  className="absolute left-[-2rem] md:static md:w-[5rem] flex items-center justify-center flex-shrink-0 z-10"
                >
                  <div className="w-10 h-10 md:w-12 md:h-12 rounded-full border-2 bg-[#050511] flex items-center justify-center text-lg md:text-xl shadow-[0_0_20px_rgba(0,0,0,0.5)] transition-transform hover:scale-110" style={{ borderColor: item.color, boxShadow: `0 0 15px ${item.color}40` }}>
                    {item.icon}
                  </div>
                </div>
                ...
                <div className="w-full flex-1">
                  <div className="p-8 md:p-10 rounded-[2rem] bg-white/[0.02] border border-white/5 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.3)] hover:bg-white/[0.04] transition-colors duration-300">
                    <p className="text-sm font-bold tracking-widest mb-3" style={{ color: item.color }}>
                      {item.year}
                    </p>
                    <h3 className="text-xl md:text-2xl font-bold text-white/90 mb-2">
                      {item.title}
                    </h3>
                    <p className="text-sm md:text-base text-white/50 mb-6 font-medium">
                      {item.institution}
                    </p>

                    <ul className="space-y-3">
                      {item.points.map((point, i) => (
                        <li key={i} className="flex items-start gap-3 text-sm md:text-base text-[#9ca3af] leading-relaxed">
                          <svg className="w-4 h-4 mt-1 flex-shrink-0" style={{ color: item.color }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
